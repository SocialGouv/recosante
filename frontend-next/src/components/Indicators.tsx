...
export default function Indicators({
  ...
  console.log(
    `Indicators: Utilisation du code commune:`,
  ...
  const indiceAtmo = IndicatorService.getIndicatorBySlug(
    indicators,
    'indice_atmospheric',
  );
  const indiceUv = IndicatorService.getIndicatorBySlug(indicators, 'indice_uv');
  const pollens = IndicatorService.getIndicatorBySlug(
    indicators,
    'pollen_allergy',
  );
  const vigilanceMeteo = IndicatorService.getIndicatorBySlug(
    indicators,
    'weather_alert',
  );
  const baignades = IndicatorService.getIndicatorBySlug(
    indicators,
    'bathing_water',
  );

  console.log('Indicators: Données récupérées:', {
    indicators,
    indiceAtmo,
    indiceUv,
    pollens,
  ...
  const sortIndicators = React.useCallback(
    (indicators: any[]) => {
      ...
      // Trier par favoris puis par ordre d'apparition
      return validIndicators.sort((a, b) => {
        ...
        // Si même statut de favori, garder l'ordre original
        return 0;
      });
    },
    [isFavorite],
  ...
  // Debug: Vérifier les indicateurs avant filtrage
  console.log('Indicators: Indicateurs avant filtrage:', {
    total: allIndicators.length,
    indicators: allIndicators.map(ind => ({
      slug: ind.slug,
      hasData: !!ind.data,
      hasComponent: !!ind.component
    }))
  });
  ...
  // Debug: Vérifier les indicateurs triés
  console.log('Indicators: Indicateurs triés:', {
    total: sortedIndicators.length,
    indicators: sortedIndicators.map(ind => ({
      slug: ind.slug,
      hasData: !!ind.data,
      isFavorite: isFavorite(ind.slug)
    }))
  });
        ...
        <div className='flex items-center justify-center py-12'>
          <div className='text-center'>
            ...
            <div className='mt-4 p-4 bg-gray-100 rounded'>
              ...
              <p className='text-xs'>
                URL API: {process.env.NEXT_PUBLIC_API_URL}
                /api/indicators/website
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <section className='mx-auto max-w-6xl'>
      ...
      {/* Rendu conditionnel basé sur le hook useScreenSize */}
      {isMobile ? (
        ...
        <div className='space-y-6'>
          ...
          {sortedIndicators.map((indicator, index) => {
            ...
            if (!indicator || !indicator.data) {
              console.warn('Indicators: Indicateur invalide détecté:', indicator);
              return null;
            }
            
            return (
              <IndicatorWithFavorite
                key={`mobile-${indicator.slug}-${index}`}
                indicator={indicator}
        ...
        >
          ...
          <div className='space-y-6'>
            {sortedIndicators
              .filter((indicator, index) => index % 2 === 0 && indicator && indicator.data)
              .map((indicator, index) => (
                <IndicatorWithFavorite
          ...
          <div className='space-y-6'>
            {sortedIndicators
              .filter((indicator, index) => index % 2 === 1 && indicator && indicator.data)
              .map((indicator, index) => (
                <IndicatorWithFavorite
