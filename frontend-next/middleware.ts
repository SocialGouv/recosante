...
// Configuration des redirections directement dans le middleware
const santeFrRedirects: Record<string, string> = {
  ...
  'quels-gestes-adopter-pour-profiter-du-sapin-de-noel-en-toute-securite-a-la-maison': 'https://www.sante.fr/quels-gestes-adopter-pour-profiter-du-sapin-de-noel-en-toute-securite-la-maison'
};
...
export function middleware(request: NextRequest) {
  ...
  // Vérifier si c'est une page d'article avec redirection
  if (pathname.startsWith('/articles/')) {
    // Extraire le slug de l'article
    const slug = pathname.replace('/articles/', '');
    
    console.log('Middleware - Slug extrait:', slug);
    console.log('Middleware - Redirections disponibles:', Object.keys(santeFrRedirects));
    
    // Vérifier s'il y a une redirection vers sante.fr
    if (santeFrRedirects[slug]) {
      const redirectUrl = santeFrRedirects[slug];
      
      console.log('Middleware - Redirection trouvée pour:', slug, 'vers:', redirectUrl);
      
      if (redirectUrl) {
        // Effectuer une redirection 301 (permanente)
        console.log('Middleware - Effectuant la redirection 301 vers:', redirectUrl);
      ...
      console.log('Middleware - Pas de redirection trouvée pour:', slug);
    }
  }
