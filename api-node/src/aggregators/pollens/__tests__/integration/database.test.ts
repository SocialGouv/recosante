...
import { AlertStatusEnum, DataAvailabilityEnum, IndicatorsSlugEnum } from '@prisma/client';
...
describe('Test pollen aggregation database functions', () => {
  ...
  beforeAll(async () => {
    ...
    console.log(`Connected to database: ${process.env.DATABASE_URL?.includes('test') ? 'TEST' : 'PRODUCTION'}`);
    ...
    testPollenRow = {
      ...
      validity_end: new Date(testDiffusionDate.getTime() + 7 * 24 * 60 * 60 * 1000),
  ...
  afterAll(async () => {
    try {
      for (const id of createdRecordIds) {
        await prisma.pollenAllergyRisk.delete({
          where: { id }
        }).catch(() => {
      ...
      if (testMunicipality?.COM) {
        await prisma.municipality.delete({
          ...
          console.log(`Note: Could not delete test municipality: ${err.message}`);
        });
      }
    } catch (error) {
  ...
  describe('insertPollensData', () => {
    ...
    it('Should handle alert notifications when alert threshold is reached', async () => {
      ...
      expect(savedData).not.toBeNull();
      expect(savedData?.alert_status).toBe(AlertStatusEnum.ALERT_NOTIFICATION_SENT);
  ...
  describe('handleAlertNotification', () => {
    it('Should update notification status to SENT on success', async () => {
      ...
      expect(updatedRow?.alert_status).toBe(AlertStatusEnum.ALERT_NOTIFICATION_SENT);
    });
    ...
    it('Should update notification status to ERROR on failure', async () => {
      ...
      expect(updatedRow?.alert_status).toBe(AlertStatusEnum.ALERT_NOTIFICATION_ERROR);
    });
  });
});
