...
const mockHandlers = require('~/getters/eventHandlers');
...
describe('EventService', () => {
  ...
  describe('processEvent', () => {
    ...
    it('should handle STORE_REVIEW event correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockHandlers.handleStoreReviewEvent).toHaveBeenCalledWith('test-user-id', mockReq);
    });
    ...
    it('should handle APP_OPEN event correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockHandlers.handleAppOpenEvent).toHaveBeenCalledWith('test-user-id', mockReq);
    });
    ...
    it('should handle FIRST_TIME_LAUNCH event correctly', async () => {
      ...
      mockHandlers.handleFirstTimeLaunchEvent.mockResolvedValue({ success: true });

      const result = await EventService.processEvent(mockReq);

      expect(result.success).toBe(true);
      expect(mockHandlers.handleFirstTimeLaunchEvent).toHaveBeenCalledWith('test-user-id', mockReq);
    });
