  ...
  const details = props?.data?.vigilance_meteo?.indice?.details
    ...
    .reduce((details, { indice }) => {
      ...
      details[indice.color][indice.label].validity.push({
        start:
          indice.validity.start && new Date(indice.validity.start.slice(0, 19)),
      ...
      {values
        ...
        .map(
          (c) =>
            details &&
            details[c] &&
            Object.keys(details[c]).map((l) => (
              <Element
                indice={details[c][l].indices[0]}
                key={
                  details[c][l].indices[0].color +
                  "-" +
                  details[c][l].indices[0].label
                }
                validity={details[c][l].validity}
              />
            ))
        )}
    </div>
