...
export default function Email(props) {
  ...
  const desabonnementData = Object.keys(props.active_users).map((key) => ({
    date: key,
    desinscriptions:
      (props.desinscriptions[key] || 0) / props.active_users[key],
  }));
  const frequenceData = [
    {
      name: "Tous les jours",
  ...
  return (
    <>
      <Section>
        <MainTitle>L’abonnement aux indicateurs</MainTitle>
        <Subsection>
        ...
        <Subsection>
          ...
          <StyledSection>
            <Text center>
              <strong>
                {formatAsPercent(
                  props.indicateur_indice_atmo / props.total_actifs
                )}
              </strong>
            ...
            <Text center>
              <strong>
                {formatAsPercent(
                  props.indicateur_vigilance_meteo / props.total_actifs
                )}
              </strong>
            ...
            <Text center>
              <strong>
                {formatAsPercent(
                  props.indicateur_indice_uv / props.total_actifs
                )}
              </strong>
              <br /> à l’indice UV
            </Text>
        ...
        <Subsection>
          ...
          <Section.Subtitle center>
            (après <strong>{props.temps_moyen_inscription}</strong> jours en
            moyenne)
          </Section.Subtitle>
          <Wrapper>
            <ResponsiveContainer>
              <LineChart data={desabonnementData}>
                ...
                <Tooltip
                  formatter={(value) => formatAsFractionPercent(value)}
                />
                <Legend />
                <Line
      ...
      <Section>
        ...
        <StyledSection>
          <Text center>
            <strong>{formatAsDecimal(props.newsletter_hebdo)}</strong>
            <br /> abonnés reçoivent l’infolettre
          </Text>
          {props.opening > 0 && (
            <Text center>
              <strong>+ de {formatAsPercent(props.opening)}</strong>
