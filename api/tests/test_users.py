...
def test_default(client, commune):
    ...
    for key, value in data.items():
        ...
        assert getattr(inscription, key) == value
    ...
    authentication_token = authenticator.decode_token(
        response.json['authentication_token'])
    assert authentication_token['uid'] == inscription.uid

    response = client.get(
        f'/users/{inscription.uid}?token={response.json["authentication_token"]}')
...
def test_list_user(db_session, client):
    listes = [
    ...
    for attribute_name in listes:
        ...
        one_of_validator = next(filter(lambda v: hasattr(
            v, 'choices'), attribute.inner.validators))
...
def test_get_user(client, commune_commited):
    _ = commune_commited
    data = {
        'mail': 'lebo@tonvelo.com',
        "commune": {
    ...
    response = client.get(
        f'/users/{uid}?token={api_authenticator.make_token(uid)}')
    assert response.status_code == 200
    assert response.json['uid'] == uid
    assert response.json['mail'] == data['mail']


def test_webpush_subscriptions_info(client, commune_commited):
    _ = commune_commited
    data = {
        'mail': 'lebo@tonvelo.com',
        "commune": {
...
def test_update_user_bad_uid(client, commune_commited):
    _ = commune_commited
    data = {
        'mail': 'lebo@tonvelo.com',
        "commune": {
    ...
    response = client.post(
        f'/users/{uid}?token={api_authenticator.make_token(uid)}', json=data)
    assert response.status_code == 404


def test_update_user(client, commune_commited):
    _ = commune_commited
    data = {
        'mail': 'lebo@tonvelo.com',
        "commune": {
    ...
    assert inscription.animaux_domestiques is None
    ...
    response = client.post(
        f'/users/{uid}', json={"animaux_domestiques": ["aucun"]})
    assert response.status_code == 401
    response = client.post(
        f'/users/{uid}?token={api_authenticator.make_token(uid)}', json={"animaux_domestiques": ["aucun"]})
    assert response.status_code == 200
    inscription = Inscription.query.filter_by(mail=data['mail']).first()
    assert inscription is not None
    ...
    response = client.post(
        f'/users/{uid}',
        json={"indicateurs_media": ["notifications_web"],
              "webpush_subscriptions_info": j_dump}
    )
    assert response.status_code == 401
    response = client.post(
        f'/users/{uid}?token={api_authenticator.make_token(uid)}',
...
def test_update_user_with_existing_email(client, commune_commited):
    _ = commune_commited
    data = {
        'mail': 'lebo@tonvelo.com',
        "commune": {
...
def test_deactivate(db_session, inscription, client):
    ...
    api_authenticator = APIAuthenticator()
    response = client.post(
        f'/users/{uid}/_deactivate?token={api_authenticator.make_token(uid)}',
