...
from marshmallow.fields import Bool, Email, List, Nested, Str
...
class User(Schema):
    ...
    activites = list_str(
        ["jardinage", "bricolage", "menage", "sport", "aucun"])
    ...
    connaissance_produit = list_str(
        ["medecin", "association", "reseaux_sociaux", "publicite", "ami", "pro", "autrement"])
    population = list_str(
        ["pathologie_respiratoire", "allergie_pollens", "aucun"])
    indicateurs = list_str(
        ["indice_atmo", "raep", "indice_uv", "vigilance_meteo"])
    indicateurs_frequence = list_str(
        ["quotidien", "hebdomadaire", "alerte"], 1)
    indicateurs_media = list_str(["mail", "notifications_web"])
    recommandations = list_str(
        ["oui", "non"], 1, attribute='recommandations_actives')
    recommandations_frequence = list_str(
        ["quotidien", "hebdomadaire", "pollution"], 1)
    recommandations_media = list_str(["mail", "notifications_web"])
    webpush_subscriptions_info = Str(
...
class Response(User, ResponseSchema):
    ...
    # allow_none=True Dans le cas d’une désinscription
    mail = Email(required=True, allow_none=True)
...
class RequestPOST(User, RequestSchema):
    ...
    def make_inscription(self, data, **kwargs):
        _ = kwargs
        inscription = Inscription.query.filter(
            Inscription.mail.ilike(data['mail'])).first()
        if inscription:
...
class RequestPOSTID(User, RequestSchema):
    def __init__(self, **kwargs):
        ...
        super().__init__(
            partial=partial_arg, **super_kwargs)
