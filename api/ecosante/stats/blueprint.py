...
def stats_web():
    ...
    switch_date = datetime(2023, 9, 1).date()
    switch_date_string = switch_date.strftime('%Y-%m-%d')

    if today_date >= switch_date:
        try:
            request = requests.get(
                f"{matomo_api_url}&method=Events.getCategory&category=Widget&secondaryDimension=eventAction&period=range&date={switch_date_string},today",
                timeout=10
            )
            request.raise_for_status()
