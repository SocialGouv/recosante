...
from flask import current_app, redirect, stream_with_context, url_for
from flask.wrappers import Response
from flask_rebar import SwaggerV3Generator
from indice_pollution import episodes as get_episodes
from indice_pollution import forecast, raep
from indice_pollution.history.models import (IndiceATMO, IndiceUv,
                                             PotentielRadon, VigilanceMeteo)
from indice_pollution.history.models.commune import Commune
from indice_pollution.history.models.episode_pollution import EpisodePollution
from sqlalchemy.orm import joinedload

from ecosante.api.baignades import make_baignades_response
from ecosante.api.schemas import (BaignadesQuerySchema, QuerySchema,
                                  RecommandationExportSchema, ResponseSchema)
from ecosante.extensions import cache, rebar
...
def index():
    ...
    indice_raep = raep(insee, date_=date_,
                       departement_as_dict=False) if show_raep else None
    ...
    indice_uv = IndiceUv.get(
        insee=insee, date_=date_) if show_indice_uv else None

    advice_atmo = get_advice(advices, "indice_atmo", qualif=indice_atmo.indice) if indice_atmo and not hasattr(
        indice_atmo, "error") else None
    advice_raep = get_advice(advices, "pollens", raep=int(
        indice_raep["data"]["total"])) if indice_raep and indice_raep.get('data') else None
    advice_radon = get_advice(
        advices, "radon", potentiel_radon=potentiel_radon.classe_potentiel if potentiel_radon else None)
    advice_episode = get_advice(advices, "episode_pollution", polluants=[
                                episode.lib_pol_normalized for episode in EpisodePollution.filter_etat_haut(episodes)])
    advice_indice_uv = get_advice(
        advices, "indice_uv", indice_uv=indice_uv.uv_j0) if indice_uv and indice_uv.uv_j0 is not None else None
...
def baignades():
    ...
    try:
        ...
        current_app.logger.error(f"Cache is unavailable: {exception}")
    if resp is None:
        # Sans cache
        resp = make_baignades_response(insee)
        try:
            baignades_cache_timeout = os.getenv(
                'BAIGNADES_CACHE_TIMEOUT', "86400")  # seconds
...
def recommandations_csv():
    _recommandations = RecommandationExportSchema(
        many=True
    ).dump(
    ...
    csv_writer.writerows(_recommandations)
    return Response(
        output.getvalue(),
        mimetype="text/csv",
        headers={
            "Content-Disposition": "attachment; filename=recommandations.csv"
        }
    )
...
def batch():
    ...
    def iterate():
        ...
        for commune_id, indice in indices:
            ...
            first = False
        yield ']'
    return Response(stream_with_context(iterate()))
