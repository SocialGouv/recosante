...
class Recommandation(db.Model):
    ...
    potentiel_radon: List[int] = db.Column(
        postgresql.ARRAY(db.Integer), nullable=True)
    vigilance_couleur_ids: List[int] = db.Column(
        postgresql.ARRAY(db.Integer), nullable=True)
    vigilance_phenomene_ids: List[int] = db.Column(
        postgresql.ARRAY(db.Integer), nullable=True)
    ...
    def _multi_setter(self, prefix, list_, values):
        if not isinstance(values, list):
            return
        for value in list_:
            setattr(self, prefix + value, value in values)
    ...
    def polluants(self, value):
        self._multi_setter(
            "", ['ozone', 'dioxyde_azote', 'dioxyde_soufre', 'particules_fines'], value)
    ...
    def qualif_categorie(qualif):
        ...
        if qualif in (['bon', 'moyen'] + ['tres_bon', 'mediocre']):
            return "bon"
        if qualif in ['degrade', 'mauvais', 'tres_mauvais', 'extrement_mauvais']:
            return "mauvais"
        if qualif == "evenement":
            return "evenement"
        return None
    ...
    def criteres(self) -> Set[str]:
        liste_criteres = ["menage", "bricolage",
                          "jardinage", "velo", "tec", "voiture", "sport"]
        return {critere for critere in liste_criteres
                if getattr(self, critere)}

    # pylint: disable-next=too-many-arguments
    def is_relevant_inscription(
            self,
            inscription: Inscription = None,
            qualif=None,
            polluants: List[str] = None,
            raep: int = None,
            potentiel_radon: int = None,
            date_: date = None,
            media: str = 'mail',
            types: List[str] = None,
            vigilances=None,
            indice_uv: int = None):
        _ = (qualif, polluants, raep, potentiel_radon,
             date_, media, types, vigilances, indice_uv)
        # Inscription
        if inscription:
    ...
    def is_relevant_qualif(self, qualif):
        ...
        if self.qa_bonne is None and self.qa_mauvaise is None and self.qa_evenement is None:
            return True
        ...
        if qualif_categorie == "mauvais" and self.qa_mauvaise:
            return True
        if qualif_categorie == "evenement" and self.qa_evenement:
            return True
        # Sinon c’est pas bon
        return False

    # pylint: disable-next=too-many-arguments,too-many-return-statements
    def is_relevant_pollens(
            self,
            inscription: Inscription = None,
            qualif=None,
            polluants: List[str] = None,
            ...
            vigilances=None,
            indice_uv: int = None):
        _ = (qualif, polluants, potentiel_radon,
             date_, types, vigilances, indice_uv)
        if self.type_ != "pollens":
            return True
        if not isinstance(self.min_raep, int) or not isinstance(raep, int):
            return False
        if self.min_raep == 0 and raep != 0:
            return False
        if self.min_raep == 1 and not 1 <= raep <= 3:
            return False
        if self.min_raep == 4 and raep < self.min_raep:
            return False
        if media != "dashboard":
            if raep > 0:
    ...
    def is_relevant_season(
            ...
            vigilances=None,
            indice_uv: int = None):
        _ = (inscription, qualif, polluants,
             raep, potentiel_radon, media, types, vigilances, indice_uv)
        if date_:
            ...
            if self.hiver and season != 1:
                return False
            if self.printemps and season != 2:
                return False
            if self.ete and season != 3:
                return False
            if self.automne and season != 4:
                return False
        return True

    # pylint: disable-next=too-many-arguments
    def is_relevant_vigilance_meteo(
            self,
            inscription: Inscription = None,
            qualif=None,
            polluants: List[str] = None,
            ...
            vigilances=None,
            indice_uv: int = None):
        _ = (inscription, qualif, polluants, raep,
             potentiel_radon, date_, media, types, indice_uv)
    ...
    def is_relevant_indice_uv(
            ...
            indice_uv: int = None):
        _ = (inscription, qualif, polluants, raep,
             potentiel_radon, date_, media, types, vigilances)
        if self.type_ != "indice_uv":
            return True
        if not isinstance(self.min_indice_uv, int) or not isinstance(indice_uv, int):
            return False
        if self.min_indice_uv == 0 and indice_uv != 0:
            return False
        if self.min_indice_uv == 1 and not 1 <= indice_uv <= 2:
            return False
        if self.min_indice_uv == 3 and not 3 <= indice_uv <= 5:
            return False
        if self.min_indice_uv == 6 and not 6 <= indice_uv <= 7:
            return False
        if self.min_indice_uv == 8 and not 8 <= indice_uv <= 10:
            return False
        if self.min_indice_uv == 11 and indice_uv < self.min_indice_uv:
            return False
        return True
    ...
    def is_relevant(
        ...
        if types is None:
            types = ["indice_atmo", "episode_pollution",
                     "pollens", "vigilance_meteo", "indice_uv"]
    ...
    @classmethod
    def shuffled(cls, user_seed=None, preferred_reco=None, remove_reco=None):
        if remove_reco is None:
            remove_reco = []
        recommandations = cls.published_query().all()
        user_seed = 1 / \
            (uuid.UUID(user_seed, version=4).int) if user_seed else random.random()
        random.Random(user_seed).shuffle(recommandations)
        recommandations = list(filter(lambda r: str(
            r.id) not in set(remove_reco), recommandations))
        if preferred_reco:
    ...
    @classmethod
    def sanitizer(cls, to_sanitize):
        if to_sanitize is None:
            return to_sanitize
        for punc in ['?', '!', ';', ':']:
            result = to_sanitize.replace(f' {punc}', f' {punc}')
        for punc in [';', ':']:
            result = result.replace(f'{punc} ', f'{punc} ')
        result = result.replace("'", '’')
        result = result.replace("  ", " ")
        result = result.strip()
        return markdown.reset().convert(to_sanitize)
